  ...
  return (
    <>
      ...
      <aside
        ...
        h-full bg-[#0B0B15] border-r border-white/5 flex flex-col flex-shrink-0
      ...
      >
        {/* Logo Area */}
        <div
          className="h-20 flex items-center px-6 border-b border-white/5 bg-[#0B0B15] cursor-pointer hover:bg-white/5 transition-colors"
          onClick={() => navigate('/')}
        >
        ...
        <div className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          ...
          <div
            ...
            className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-bold bg-[#1f6b7a]/10 text-[#1f6b7a] border border-[#1f6b7a]/20 shadow-sm transition-all cursor-pointer"
          >
...
const JobCard = ({ logo, role, company, tags, match, salary }) => (
  <div className="bg-[#151A25] rounded-xl border border-white/5 p-5 relative overflow-hidden group hover:border-[#1f6b7a]/30 transition-all duration-300 shadow-md">
    ...
    <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5">
      <div className="flex-shrink-0">
        ...
        <div className="w-16 h-16 rounded-lg bg-[#151A25] border border-white/10 flex items-center justify-center text-xl text-white shadow-lg group-hover:scale-105 transition-transform duration-300 p-0">
      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span key={tag} className="px-2.5 py-1 rounded bg-[#151A25] border border-white/5 text-[10px] font-medium text-gray-300">{tag}</span>
          ))}
          {salary && <span className="px-2.5 py-1 rounded bg-[#151A25] border border-white/5 text-[10px] font-medium text-green-400 border-green-400/10">{salary}</span>}
        </div>
      </div>
...
const StatCard = ({ icon, label, value, subtext, progress }) => (
  <div className="bg-[#151A25] rounded-xl border border-white/5 p-5 relative overflow-hidden group hover:border-[#1f6b7a]/30 transition-all shadow-md">
    <div className="relative z-10">
      ...
      {progress ? (
        <div className="w-full bg-[#151A25] h-1.5 rounded-full overflow-hidden">
...
export default function Dashboard() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex w-screen h-screen bg-[#0B0B15] text-white font-sans overflow-hidden selection:bg-[#1f6b7a] selection:text-white">
      {/* Sidebar for large screens & mobile drawer */}
      <Sidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />

      {/* Hamburger Button - only on mobile (now top right) */}
      <button
        className="fixed top-4 right-4 z-[60] flex items-center justify-center w-10 h-10 bg-[#151A25] rounded-lg border border-white/10 lg:hidden hover:bg-[#151A25]/80 transition-colors focus:outline-none shadow"
      ...
      {/* Main Content Column */}
      <div className="flex-1 flex flex-col h-full relative bg-[#0B0B15] ml-0 lg:ml-0">
        {/* Ambient Glow */}
        ...
        {/* Header */}
        <header className="h-20 flex-shrink-0 flex items-center justify-between px-3 sm:px-6 md:px-8 bg-[#0B0B15]/95 backdrop-blur-md border-b border-white/5 relative z-20">
          <div className="w-1/3 min-w-0 flex items-center">
            <div className="relative hidden md:block group w-full max-w-xs">
              ...
              <input
                className="w-full bg-[#151A25] border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-xs text-white focus:ring-1 focus:ring-[#1f6b7a] focus:border-[#1f6b7a] placeholder-gray-600 transition-all outline-none"
            ...
            <div className="grid grid-cols-1 gap-8 h-full lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 space-y-5">
                ...
                <div className="space-y-4">
                  <JobCard logo="diamond" role="Senior Frontend Engineer" company="AcmeCorp â€¢ Remote" tags={['React', 'TypeScript']} salary="$140k - $180k" match="98" />
                  <JobCard logo="change_history" role="Product Designer" company="Vertex Inc. â€¢ New York" tags={['Figma', 'UI/UX']} match="94" />
                  <JobCard logo="hexagon" role="ML Engineer" company="HexaTech â€¢ Austin, TX" tags={['Python', 'PyTorch']} salary="$160k+" match="89" />
                  <JobCard logo="code_blocks" role="Full Stack Developer" company="StackFlow â€¢ Remote" tags={['Next.js', 'Node']} match="82" />
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#151A25] rounded-xl border border-white/5 p-5 relative overflow-hidden shadow-md">
                  ...
                  <div className="bg-[#151A25] rounded-lg p-4 border border-white/5">
